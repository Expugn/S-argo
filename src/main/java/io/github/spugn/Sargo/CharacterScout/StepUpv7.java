...
public class StepUpv7 extends CharacterScout
{
    ...
    public StepUpv7(Message message, int bannerID, String choice, String discordID)
    {
    ...
    protected void setupScoutMenu()
    {
        if (!SIMPLE_MESSAGE)
        {
            switch (CHOICE.toLowerCase())
            {
                ...
                case "m":
                case "mi":
                    sMenu = sMenu.andThen(s -> s.setTitle("[Step Up v7] - Step " + bannerTypeData));
                    break;
                default:
                    sMenu = sMenu.andThen(s -> s.setTitle("[Step Up v7] - Unknown"));
                    break;
            }
        }
        else
